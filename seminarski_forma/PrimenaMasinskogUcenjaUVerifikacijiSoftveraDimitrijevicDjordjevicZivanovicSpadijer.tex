...
% vvvv FIXME IZUZETNO ROGOBATNO vvvv 
...
Najmanje frekfentna
Algoritam za odabir akcije REINFORCEMENT LEARNING moramo modifikovati zbog problema sa nasumičnim odabirom. Modifikovan algoritam koristi delove anti-ant algoritma kako bi izbegao generacije redudantnih testova.
...
Instanciranje p[$\bar{x}$] ulaznim vrednostima $\bar{v}$ odgovarajuceg tipa, obeležava se p[$\bar{v}$]. % Given values v¯ = v1 . . . , vn we write ϕ[¯v] for the replacement of xi in ϕ by vi for 1 ≤ i ≤ n. 
...
Kada je p deterministicko, [[p]] smatramo funkcijom [[p]]: $Stanja \times Vrednost^n  \rightarrow Stanja$
