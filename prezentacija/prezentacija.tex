...
% mislim da je bolje ova slika nego tekst
...
%%%%%%%%%%% mislim da nam nije potreban ovaj slajd, na osnovu gornjeg se moze sve ispricati
%\begin{frame}
%\frametitle{Evaluacija modela i mere kvaliteta}
%\begin{itemize}
...
\begin{itemize}
	...
	\item Testiranja bazirana na modelu (eng. \emph{model-based testing})
	\begin{itemize}
		\item Implementacija pod testom \newline (eng. \emph{implementation under test - IUT})
		\item Model	
	\end{itemize}
\end{itemize}
...
Čuvano pravilo ažuriranja je par ($\varphi$,p) 
\end{frame}
...
\begin {itemize}
	...
	\item Inače: \newline \textbf{izaberi izmedu kontrolisane i posmatrane akcije.}
\end {itemize}
...
\end {itemize}
\textbf{učenje uslovljavanjem, anti-ant algoritam} \newline
Akcije $(a_i)_{i < k}$, Cene $(c_i)_{i < k}$\newline
verovatnoća za izbor akcije $a_i$ biti $c_i^{-1} / \sum_{j=0}^{k} c_j^{-1}$.
